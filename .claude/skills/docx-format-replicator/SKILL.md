...
This project has a shared template library at `d:/项目/.claude/templates/`.

**Always check the library first before asking the user for a template.**

### How to use the library:

**List available templates:**
```bash
python d:/项目/.claude/templates/scan.py list
```

**Add a new template:**
1. User places `.docx` template into `d:/项目/.claude/templates/originals/`
2. Run: `python d:/项目/.claude/templates/scan.py`
3. The format is auto-extracted and added to the index

**Use a template from the library:**
- Format JSON is at `d:/项目/.claude/templates/formats/<name>.json`
- Skip the extraction step, go directly to content preparation and generation
- When user says "用 XX 模板的格式", match against names in the index

**The index file** (`d:/项目/.claude/templates/index.json`) contains all template metadata.
